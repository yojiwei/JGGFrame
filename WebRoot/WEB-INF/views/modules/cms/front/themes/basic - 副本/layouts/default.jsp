				...
				<div class="nav-collapse">
					<ul id="main_nav" class="nav nav-pills">
						<li class="${not empty isIndex && isIndex ? 'active' : ''}"><a
							href="${ctx}/index-1${fns:getUrlSuffix()}"><span>${site.id
									eq '1'?'首 页':'返回主站'}</span></a></li>
						<!-- 导航栏目 -->
						...
						<!-- 多个站点 -->
						<li id="siteSwitch" class="dropdown" style="display:none;"><a
						...
						<!-- 切换网站风格 -->
						<li id="themeSwitch" class="dropdown" style="display:;"><a
							class="dropdown-toggle" data-toggle="dropdown" href="#"
					...
					<form class="navbar-form pull-right" style="display:none;" action="${ctx}/search"
						method="get">
						<input type="text" name="q" maxlength="20" style="width: 65px;"
							placeholder="全站搜索..." value="${q}">
					</form>
					<div class="site-date">${fns:getDate('yyyy年MM月dd日 E')}</div>
				</div>
				<!--/.nav-collapse -->
			</div>
		</div>
	...
	<div class="container">
		...
		<footer>
			<div class="footer_nav">
				金桥政务&nbsp;&nbsp;
				<!-- <a href="${ctx}/guestbook" target="_blank">公共留言</a> | <a href="${ctx}/search" target="_blank">全站搜索</a> | <a href="${ctx}/map-${site.id}${fns:getUrlSuffix()}" target="_blank">站点地图</a> | <a href="mailto:test@163.com">技术支持</a> |  -->
				<a href="${pageContext.request.contextPath}${fns:getAdminPath()}"
