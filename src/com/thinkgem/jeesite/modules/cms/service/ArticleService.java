...
//		return dao.find(page, dc);
	...
	public void save(Article article) {
		...
		if (StringUtils.isBlank(article.getId())){
			...
			dao.insert(article);//保存文章信息
			articleDataDao.insert(articleData);//保存文章内容信息
		}else{
			article.preUpdate();
			articleData = article.getArticleData();
			articleData.setId(article.getId());
			dao.update(article);//修改文章信息
			articleDataDao.update(article.getArticleData());//修改文章内容信息
		}
	}
...
//		dao.updateDelFlag(id, isRe!=null&&isRe?Article.DEL_FLAG_NORMAL:Article.DEL_FLAG_DELETE);
	 ...
	 * @return
	 */
	public List<Object[]> getFiles(Article e){
		if(e.getFiles() == null){
			return new ArrayList<Object[]>();
		}
		List<Object[]> list = Lists.newArrayList();
		String[] filess = e.getFiles().split("\\|");
		String filess_path = "";
